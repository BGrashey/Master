...
class Measurements:
    ...
    def __init__(self,
        ...
        self.ap_corr = min(self.aperture_correction(), 5)
        ...
        self.cont_ratio = self.cont / self.cont_controll
    ...
    def cog(self, rmax=15):
        ...
        line_cube = self.cube.data[mask, :, :]

        line_map = np.sum(line_cube, axis=0)

        y, x = np.indices(line_map.shape)
        r = np.sqrt((x - self.x0)**2 + (y - self.y0)**2)

        mask_rmax = r <= rmax
        r_values = r[mask_rmax]
        flux_values = line_map[mask_rmax]
        
        sort_idx = np.argsort(r_values)
        r_sorted = r_values[sort_idx]
        flux_sorted = flux_values[sort_idx]
        
        cumulative_flux = np.cumsum(flux_sorted) * 1e-17
        
        radii = np.arange(1, rmax + 1)
        fluxes = []
        for rad in radii:
            idx = np.searchsorted(r_sorted, rad, side='right')
            if idx > 0:
                fluxes.append(cumulative_flux[idx-1])
            else:
                fluxes.append(0)
    ...
    def flux_tot(self, fluxes, tol=0.005):
       
        diffs = np.diff(fluxes)
        rel_change = np.abs(diffs / fluxes[:-1])
        
        idx = np.where(rel_change < tol)[0]
        
        if len(idx) > 0:
            best_idx = idx[0] + 1
            return fluxes[best_idx]
        else:
...
"""        
    ...
    def fit_model(self, width=200):
        ...
        smooth_reg = ((wave > (center_guess - 3 * width)) &
                        (wave < (center_guess + 3 * width))
                        )

        
        max_half = max(self.flux[fit_region])
        ...
        smooth_fit = median_filter(self.flux[smooth_reg], size=smooth_len)
        wave_smooth = wave[smooth_reg]
        wave_fit = wave[fit_region]
        ...
        fitter_pol = fitting.LinearLSQFitter()
        fitter = fitting.LevMarLSQFitter()
        
        good_pixels = np.isfinite(smooth_fit)

        best_cont = fitter_pol(
            polynomial,
            wave_smooth[good_pixels],
            smooth_fit[good_pixels]
        )
        
        for param_name in polynomial.param_names:
            parameter = getattr(polynomial, param_name)
            parameter.fixed = True

        model = gauss + best_cont
    ...
    def error_estimation(self, acc=500):
        ...
        noise_sigma = np.std(self.flux[self.cont_mask]) #np.std(residuals)
    ...
    def measure_ew(self):
        ...
        return ew, ew_err, ew_HSC, ew_HSC_err, flux_line, flux_error, redshift, cont, cont_err, cont_hsc, cont_hsc_err, self.detection
